...
interface AuthCtx {
  ...
  login: (ssoSubject: string) => Promise<void>;
  logout: () => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (ssoSubject: string) => {
    await api.login(ssoSubject);
    const s = authStore.get();
    setUser({
      ssoSubject: s.ssoSubject,
      userId: s.userId,
  ...
  return (
    <AuthContext.Provider value={{ user, loading, memberships, login, logout, refreshMemberships }}>
      {children}
